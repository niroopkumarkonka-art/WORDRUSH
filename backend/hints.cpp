...
public:
    ...
    void prepareHints(const std::string& secretWord) {
        ...
        for (char ch : secretWord) {
            ...
            if (u == 'A' || u == 'E' || u == 'I' || u == 'O' || u == 'U') {
                if (vowelsFound.find(u) == std::string::npos) {
                    if (!vowelsFound.empty()) vowelsFound += ", ";
                    vowelsFound += u;
                }
            }
        }
        if (vowelsFound.empty()) vowelsFound = "None";
        ...
        if (maxAllowedHints >= 2) {
            ...
            std::string pattern = "";
            pattern += firstChar;
            for (int i = 1; i < len - 1; ++i) pattern += " _";
            pattern += " ";
            pattern += lastChar;
            h2.clueText = "Ends with '" + std::string(1, lastChar) + "' (Pattern: " + pattern + ")";
        ...
        if (maxAllowedHints >= 1) {
            ...
            h1.clueText = "Starts with '" + std::string(1, firstChar) + "' (" + std::to_string(len) + " letters)";
    ...
    bool getNextHint(Hint& outHint) {
        if (!hasAvailableHints()) return false;
...
int main() {
    std::cout << "--- WordRush C++ Hints Test (1.5s Pop-up Duration) ---" << std::endl;
    ...
    while (manager.getNextHint(hint)) {
        std::cout << "Popped Hint Level " << hint.level << " [" << hint.title << "]: " 
                  << hint.clueText << " (Pop-up Display: " << hint.durationMs << "ms / "
